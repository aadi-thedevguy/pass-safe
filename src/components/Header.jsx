  ...
  return (
    <header className="w-full mx-auto mb-16">
      <nav className="flex items-center justify-between mb-20 relative">
        <div className="ml-4 ">
          ...
          {install && (
            <button
              type="button"
              className="border-2 text-accent-color btn border-accent-color py-1 mt-2 px-4 rounded-lg absolute left-2"
              onClick={install}
            >
              Install <GoDesktopDownload className="inline" />
            </button>
          )}
        </div>
        <div className="mr-6">
          <h1 className="text-4xl text-accent-color uppercase font-bold">
            Pass Safe
          </h1>
          <div
            title="Toggle Darkmode"
            className={` mt-3 flex bg-accent-color w-[70px] h-[30px] rounded-full absolute right-5
            ${!darkTheme ? "justify-end" : "justify-start"}
            `}
          >
            <span
              className="w-[40px] h-full inline-flex items-center justify-center bg-white rounded-full cursor-pointer transition-all duration-700 ease-out"
              onClick={handleMode}
            >
              {!darkTheme ? (
      ...
      <div className=" mt-6 mx-auto w-full text-primary-color dark:text-white">
        <h2 className="text-2xl text-center text-emphasis mb-5 font-bold uppercase italic">
          We Provide Passwords that
        </h2>
